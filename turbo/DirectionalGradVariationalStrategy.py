...
class DirectionalGradVariationalStrategy(_VariationalStrategy):
    ...
    def _cholesky_factor(self, induc_induc_covar):
        ...
        dense_mat = induc_induc_covar.to_dense().double()
        # eigvals = torch.linalg.eigvalsh(dense_mat)
        # print("[DirectionalGradVariationalStrategy] Min eigenvalue before Cholesky:", eigvals.min().item())
        # print("[DirectionalGradVariationalStrategy] Max eigenvalue before Cholesky:", eigvals.max().item())
    ...
    def prior_distribution(self):
        ...
        res = MultivariateNormal(zeros, torch.diag_embed(ones))
        return res
    ...
    def forward(self, x, inducing_points, inducing_values,variational_inducing_covar=None, **kwargs):
        ...
        ##### Careful, chatgpt fixed this below
        interp_term = torch.cholesky_solve(induc_data_covar.double(), L).to(full_inputs.dtype)
        # term K_ZZ^{-1/2} K_XZ^T 
        interp_term_trans = torch.cholesky_solve(data_induc_covar.transpose(-1,-2).double(), L).to(full_inputs.dtype)
        ...
        if trace_mode.on():
            ...
            predictive_covar = data_data_covar.add_jitter(1e-4) + interp_term_trans.transpose(-1, -2) @ (middle_term @ interp_term)
    ...
    def __call__(self, x, prior=False, **kwargs):
        if not self.updated_strategy.item() and not prior:
            with torch.no_grad():
                ...
                R = (L.inv_matmul(covar_root).to(variational_dist.loc.dtype))
                whitened_covar = R @ R.transpose(-1, -2)
